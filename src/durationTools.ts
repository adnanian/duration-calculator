...
/**
 * The four mathematical operators. "N/A" is included to accomodate for the first Duration
 * in the array of CalcWrappers.
 */
export type Operator = "+" | "-" | "×" | "÷" | "N/A";
...
export class Duration extends Calculable {
  ...
  private static readonly MILLIS_PER_MINUTE =
    this.MILLIS_PER_SECOND * this.SECONDS_PER_MINUTE;
  /** The number of milliseconds in an hour; SHOULD BE 3,600,000. */
  private static readonly MILLIS_PER_HOUR =
    this.MILLIS_PER_MINUTE * this.MINUTES_PER_HOUR;
  /** The number of seconds in an hour; SHOULD BE 3,600. */
  private static readonly SECONDS_PER_HOUR =
    this.SECONDS_PER_MINUTE * this.MINUTES_PER_HOUR;
  ...
  plus(durationToAdd: Duration): Duration {
    ...
    const secondsSum: number =
      Math.floor(millisSum / Duration.MILLIS_PER_SECOND) +
      this.seconds +
      durationToAdd.seconds;
    ...
    const minutesSum: number =
      Math.floor(secondsSum / Duration.SECONDS_PER_MINUTE) +
      this.minutes +
      durationToAdd.minutes;
    const minutesPlace: number = minutesSum % Duration.MINUTES_PER_HOUR;
    // Add the hours place.
    const hoursPlace: number =
      Math.floor(minutesSum / Duration.MINUTES_PER_HOUR) +
      this.hours +
      durationToAdd.hours;
  ...
  minus(durationToSubtract: Duration): Duration {
    ...
    const millisDiff: number =
      this.milliseconds - durationToSubtract.milliseconds;
    const millisPlace: number =
      millisDiff >= 0 ? millisDiff : millisDiff + Duration.MILLIS_PER_SECOND;
    // Subtract the seconds place, borrow a full minute, if difference < 0.
    const secondsDiff: number =
      Math.floor(millisDiff / Duration.MILLIS_PER_SECOND) +
      this.seconds -
      durationToSubtract.seconds;
    const secondsPlace: number =
      secondsDiff >= 0
        ? secondsDiff
        : secondsDiff + Duration.SECONDS_PER_MINUTE;
    // Subtract the minutes place, borrow a full hour, if difference < 0.
    const minutesDiff: number =
      Math.floor(secondsDiff / Duration.SECONDS_PER_MINUTE) +
      this.minutes -
      durationToSubtract.minutes;
    const minutesPlace: number =
      minutesDiff >= 0 ? minutesDiff : minutesDiff + Duration.MINUTES_PER_HOUR;
    // Subtract the hours place.
    const hoursPlace: number =
      Math.floor(minutesDiff / Duration.MINUTES_PER_HOUR) +
      this.hours -
      durationToSubtract.hours;
    return new Duration(hoursPlace, minutesPlace, secondsPlace, millisPlace);
  }
  ...
  dividedBy(scale: Scale): Duration {
    ...
    // Step 5
    const secondsSum: number =
      Math.floor(millisQuotient / Duration.MILLIS_PER_SECOND) + secondsPlace;
    secondsPlace = secondsSum % Duration.SECONDS_PER_MINUTE;
    // Step 6
    const minutesSum: number =
      Math.floor(secondsQuotient / Duration.SECONDS_PER_MINUTE) + minutesPlace;
    minutesPlace = minutesSum % Duration.MINUTES_PER_HOUR;
    // Step 7
    hoursPlace =
      Math.floor(minutesSum / Duration.MINUTES_PER_HOUR) + hoursPlace;
   ...
   */
  private readonly operationMap: Record<
    Operator,
    (calculable: any) => Duration
  > = {
    "+": this.plus.bind(this),
    ...
    // For debugging
    "N/A": (calculable: any) => {
      throw new Error("Please place conditionals before calling operationMap.");
    },
  };
   ...
   */
  performCalculation(argument: Calculable, opSymbol: Operator): Duration {
   ...
   */
  toHours(): number {
    const minutesToPartialHour: number =
      this.minutes / Duration.MINUTES_PER_HOUR;
    const secondsToPartialHour: number =
      this.seconds / Duration.SECONDS_PER_HOUR;
    const millisToPartialHour: number =
      this.milliseconds / Duration.MILLIS_PER_HOUR;
    return (
      this.hours +
      minutesToPartialHour +
      secondsToPartialHour +
      millisToPartialHour
    );
  }
  ...
  toMinutes(): number {
    ...
    const secondsToPartialMinute: number =
      this.seconds / Duration.SECONDS_PER_MINUTE;
    const millisToPartialMinute: number =
      this.milliseconds / Duration.MILLIS_PER_MINUTE;
    return (
      hoursToMinutes +
      this.minutes +
      secondsToPartialMinute +
      millisToPartialMinute
    );
  }
  ...
  toSeconds(): number {
    ...
    const millisToPartialSecond: number =
      this.milliseconds / Duration.MILLIS_PER_SECOND;
    return (
      hoursToSeconds + minutesToSeconds + this.seconds + millisToPartialSecond
    );
  }
  ...
  toMilliseconds(): number {
    ...
    return (
      hoursToMillis + minutesToMillis + secondsToMillis + this.milliseconds
    );
  }

  // Cloning method for deep copy
  clone(): Duration {
    const cloned = new Duration(
      this.hours,
      this.minutes,
      this.seconds,
      this.milliseconds
...
/**
 * A container for a Duration/Scale and mathematical operator.
...
export class CalcWrapper {
   ...
   */
  constructor(operator: Operator, durationCalculable: Calculable) {
  ...
  clone() {
    ...
    const clonedInstance = new CalcWrapper(this.operator, clonedCalculable);
